...
\begin{itemize}
  \item Deterministic performance. The algorithm needs to have a known performance for a given set of signatures and inputs. Without a known performance the algorithm has the chance to slow down the entire packet inspection operation.
  \item Memory efficiency. An algorithm needs to be memory efficient. Like with time, the algorithm must occupy a reasonable memory footprint 
  \item Dynamic update.
  \item Signatures.
  \item Scalability.
\end{itemize}
